...
namespace LurnApp.Android.Activities
{
	/// <summary>
	/// History Activity
	/// </summary>
		...
		async void SetFrameFragment (int index)
		{
			...
			case "Summary":
				var summaryFragment = new SummaryFragment {
					Assignment = historyViewModel.PastAssignment
				};
				transaction.SetTransition (FragmentTransit.FragmentOpen);
			...
			case "Items":
				...
				itemFragment.Assignment = historyViewModel.PastAssignment;
				await itemViewModel.LoadAssignmentItemsAsync (historyViewModel.PastAssignment);
				RunOnUiThread (() => {
			...
			case "Labor Hours":
				...
				await laborViewModel.LoadLaborHoursAsync (historyViewModel.PastAssignment);
				RunOnUiThread (() => {
					laborHourFragment.LaborHours = laborViewModel.LaborHours;
					laborHourFragment.Assignment = historyViewModel.PastAssignment;
					transaction.SetTransition (FragmentTransit.FragmentOpen);
			...
			case "Confirmations":
				...
				await photoViewModel.LoadPhotosAsync (historyViewModel.PastAssignment);
				RunOnUiThread (() => {
					confirmationFragment.Photos = photoViewModel.Photos;
					confirmationFragment.Assignment = historyViewModel.PastAssignment;
					transaction.SetTransition (FragmentTransit.FragmentOpen);
			...
			case "Expenses":
				...
				await expenseViewModel.LoadExpensesAsync (historyViewModel.PastAssignment);
				RunOnUiThread (() => {
					expenseFragment.Expenses = expenseViewModel.Expenses;
					expenseFragment.Assignment = historyViewModel.PastAssignment;
					transaction.SetTransition (FragmentTransit.FragmentOpen);
